...
client.on('message', async msg => {
    ...
    if (!chamados[remetente] && /^(iniciar|boa noite|boa dia|boa tarde|Olá|olá|oboai||ola|oii|dia|tarde|noite|bom|hello|oie|eai|prezados|prezado|opa|caros|migues|rony|prince|elihyago|fpe|johnny|guacira|paulo|suporte|envision|tecnologia|bot|envisionbot|OI|Oi|oI|Olá|OLÁ|OLA|)$/i.test(msg.body)) {
        await sendMessageWithDelay(remetente, `Olá, ${name}! 😊\nBem-vindo ao EnvisionBot! 🚀\n\nPor favor, informe o número do seu chamado (5 dígitos).`);
        await sendMessageWithDelay(remetente, `Caso não tenha um número de chamado, envie uma email para suporte@envisionte.com.br ou acesso o link abaixo\n https://envisiontecnologia.octadesk.com/login`);
        return;
    }
